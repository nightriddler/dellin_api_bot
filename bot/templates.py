from datetime import datetime

import jinja2
from contains import TEMPLATES_DIR


def render_template(template_name: str, data: dict | None = None) -> str:
    """Создание и обработка шаблона."""
    if data is None:
        data = {}
    template = _get_template_env().get_template(template_name)
    rendered = template.render(**data).replace("\n", " ")
    rendered = rendered.replace("&#34;", '"')
    rendered = rendered.replace("<br>", "\n")
    rendered = "\n".join(line.strip() for line in rendered.split("\n"))
    return rendered


def short_date(date: str, format="%Y-%m-%d %H:%M:%S", endformat="%d.%m.%Y") -> str:
    """Фильтр для корректировки формата даты."""
    value_dt = datetime.strptime(date, format)
    return value_dt.strftime(endformat)


def _get_template_env() -> jinja2.Environment:
    """Получить шаблон."""
    if not getattr(_get_template_env, "template_env", None):
        template_loader = jinja2.FileSystemLoader(searchpath=TEMPLATES_DIR)
        env = jinja2.Environment(
            loader=template_loader,
            trim_blocks=True,
            lstrip_blocks=True,
            autoescape=True,
        )
        env.filters["short_date"] = short_date
        _get_template_env.template_env = env

    return _get_template_env.template_env
